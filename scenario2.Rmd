...
#as = age+iq+edu+smoke
coef.as = rbind(0.002,0.015,0.02,0.1) 
# mn = age+edu+iq+smoke
coef.mn = rbind(0.043,0.02,0.02,0.02)
# pb = age+edu+iq+smoke
coef.pb = rbind(0.058,0.01,0.02,0.078)
...
mn = cbind(age,edu,iq,smoke) %*% coef.mn + rnorm(npop,0,r.err)
pb = cbind(age,edu,iq,smoke) %*% coef.pb + rnorm(npop,0,r.err)
#y
...
#scenario1
write.csv(df.pop,'./data/pop_scenario2.csv')
...
form2 = 'mn~age+iq+as.factor(edu)+as.factor(smoke)'
form3 = 'pb~age+iq+as.factor(edu)+as.factor(smoke)'
...
write.csv(man_trun,'./data/lower_truncation_mann_sce2.csv')
```
...
write.csv(man_trun,'./data/upper_truncation_mann_sce2.csv')
```
